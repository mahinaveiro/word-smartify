-- Fix the canonical mock-test finalizer's PL/pgSQL output-column ambiguity.
-- The function returns a column named `id`, so all table references are explicitly qualified.
...
begin
  ...
  if v_test.time_taken_seconds is not null then
    return query
      select false,
             v_test.id,
             v_test.user_id,
             v_test.total_questions,
             v_test.correct_answers,
             v_test.score,
             v_test.time_taken_seconds,
             v_test.created_at;
    return;
  end if;

  select count(*)::integer,
         count(*) filter (where answers.user_answer is not null)::integer
    into v_answer_count, v_answered_count
  from public.mock_test_answers as answers
  where answers.test_id = p_test_id;

  select count(*)::integer
    into v_question_count
  from public.mock_test_answers as answers
  join public.quiz_questions as questions
    on questions.id = answers.question_id
  where answers.test_id = p_test_id;

  if v_answer_count <> v_test.total_questions
     or v_question_count <> v_test.total_questions then
    raise exception 'This mock test does not contain a complete unique question set.' using errcode = '23514';
  end if;

  update public.mock_test_answers as answers
  ...
  from public.mock_test_answers as answers
  where answers.test_id = p_test_id
    and answers.user_answer is not null
  ...
  update public.mock_tests as tests
     set correct_answers = v_correct,
         score = v_score,
         time_taken_seconds = p_time_taken_seconds
   where tests.id = p_test_id
     and tests.time_taken_seconds is null
   returning tests.* into v_updated;
  ...
  return query
    select true,
           v_updated.id,
           v_updated.user_id,
           v_updated.total_questions,
           v_updated.correct_answers,
           v_updated.score,
           v_updated.time_taken_seconds,
