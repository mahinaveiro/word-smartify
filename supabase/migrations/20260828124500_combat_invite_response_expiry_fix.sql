-- Preserve expired-invite cleanup: PL/pgSQL exceptions roll back the whole function transaction.
-- Return the expired match so the service layer can present the user-facing error.
...
begin
  ...
  if v_match.status <> 'waiting' or v_match.opponent_id is not null then
