...
const PRESET_COPY: Record<CombatPreset, { title: string; detail: string; questions: string }> = {
  sprint: { title: 'Sprint', detail: 'Fast and focused', questions: '5 questions' },
  standard: { title: 'Standard', detail: 'A proper head-to-head', questions: '10 questions' },
  custom: { title: 'Custom', detail: 'Tune the length', questions: '3–20 questions' },
}
...
export function CombatView() {
  ...
  const [levelFromInput, setLevelFromInput] = React.useState('1')
  const [levelToInput, setLevelToInput] = React.useState('20')
  ...
  const parseLevelInput = (value: string): number | null => {
    if (!/^\d+$/.test(value.trim())) return null
    const parsed = Number(value)
    return Number.isInteger(parsed) && parsed >= 1 && parsed <= 104 ? parsed : null
  }
  const levelFrom = parseLevelInput(levelFromInput)
  const levelTo = parseLevelInput(levelToInput)
  const levelSourceValid = questionSource.mode !== 'level' || (levelFrom !== null && levelTo !== null)
  const [relationshipOverrides, setRelationshipOverrides] = React.useState<Record<string, { relationship: NonNullable<SocialProfile['relationship']>; relationship_id: string | null }>>({})
  const visibleSearchResults = (searchResults.data ?? []).map((profile) => {
  ...
  const createMatch = async (selectedPreset: CombatPreset = preset, selectedWager: 0 | 100 = wagerXp, recipientId?: string) => {
    ...
    if (questionSource.mode === 'level' && (levelFrom === null || levelTo === null)) {
      setError('Enter whole-number levels from 1 to 104.')
      return
    }
    const source: CombatQuestionSource = questionSource.mode === 'level'
      ? { mode: 'level', level_from: Math.min(levelFrom as number, levelTo as number), level_to: Math.max(levelFrom as number, levelTo as number) }
      : questionSource.mode === 'book'
        ? { mode: 'book', ...(bookId ? { book_id: bookId } : {}) }
  ...
  const respondToInvite = async (invite: CombatInvite, response: 'accepted' | 'declined') => {
    await run(`invite-${invite.id}`, async () => {
      ...
      try {
        match = await postCombat<CombatMatch | null>({ action: 'respond_invite', inviteId: invite.id, response })
      } finally {
  ...
  const respondToRequest = async (id: string, response: 'accepted' | 'declined' | 'cancelled') => {
    const affectedProfile = visibleSearchResults.find((profile) => profile.relationship_id === id)
  ...
  const addFriend = async (profile: SocialProfile) => {
    await run(`add-${profile.id}`, async () => {
      const friendship = await postSocial<Friendship>({ action: 'send_request', userId: profile.id })
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 pb-28 md:gap-6 md:pb-10">
      ...
      <Modal open={Boolean(notice)} onClose={() => setNotice(null)} title="Combat update" className="max-w-sm">
        <div role="status" className="flex items-start gap-3 rounded-md border-2 border-mint bg-mint/15 p-3 text-sm font-semibold"><Check className="mt-0.5 size-4 shrink-0" aria-hidden /><p>{notice}</p></div>
      </Modal>
      ...
      <Modal className="max-h-[88svh] max-w-[min(92vw,30rem)] overflow-y-auto" open={createOpen} onClose={() => { setCreateOpen(false); setFriendToChallenge(null) }} title={friendToChallenge ? `Challenge ${friendToChallenge.other_user.display_name}` : 'Create a private match'} description={friendToChallenge ? 'Choose the rules together.' : 'Pick the rules and question source.'} footer={<><Button variant="ghost" size="sm" onClick={() => { setCreateOpen(false); setFriendToChallenge(null) }}>Cancel</Button><Button size="sm" onClick={() => void createMatch(preset, wagerXp, friendToChallenge?.other_user.id)} loading={busy === 'create'} disabled={!levelSourceValid || questionSource.mode === 'book' && !bookId || questionSource.mode === 'letter' && !letter}>{friendToChallenge ? 'Send challenge' : 'Create match'} <ArrowRight className="size-4" aria-hidden /></Button></>}>
        <div className="grid gap-3">
          ...
          {questionSource.mode === 'level' ? <div className="grid gap-2 rounded-md border-2 border-foreground/15 bg-muted/40 p-3"><div className="grid grid-cols-2 gap-2"><div><Label htmlFor="combat-level-from">From level</Label><Input id="combat-level-from" type="number" min={1} max={104} value={levelFromInput} onChange={(event) => setLevelFromInput(event.target.value)} /></div><div><Label htmlFor="combat-level-to">To level</Label><Input id="combat-level-to" type="number" min={1} max={104} value={levelToInput} onChange={(event) => setLevelToInput(event.target.value)} /></div></div>{!levelSourceValid ? <p className="text-xs font-semibold text-coral-foreground">Enter whole-number levels from 1 to 104.</p> : null}</div> : null}
...
function InviteRow({ invite, busy, onRespond }: { invite: CombatInvite; busy: boolean; onRespond: (response: 'accepted' | 'declined') => void }) {
  const preset = invite.match?.preset ?? 'sprint'
  const presetLabel = PRESET_COPY[preset].title
...
function CircleSection({ invites, invitesLoading, friends, requests, busy, onRespondInvite, onRespondRequest, onChallenge }: { invites: CombatInvite[]; invitesLoading: boolean; friends: Friendship[]; requests?: { incoming: Friendship[]; outgoing: Friendship[] }; busy: string | null; onRespondInvite: (invite: CombatInvite, response: 'accepted' | 'declined') => void; onRespondRequest: (id: string, response: 'accepted' | 'declined' | 'cancelled') => void; onChallenge: (friend: Friendship) => void }) {
  return <div className="grid gap-5">
    <Card>
      ...
      <CardContent>{friends.length ? <div className="grid gap-2">{friends.map((friend) => <div key={friend.id} className="flex items-center justify-between gap-3 rounded-md border-2 border-foreground/10 p-3"><Link href={`/profile/${friend.other_user.id}?from=combat`} className="flex min-w-0 items-center gap-3 rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-foreground"><span className="relative"><Avatar name={friend.other_user.display_name} avatarId={friend.other_user.avatar_id} avatarUrl={friend.other_user.avatar_url} size="sm" />{friend.other_user.presence === 'online' || friend.other_user.presence === 'in_combat' ? <span className="absolute -bottom-0.5 -right-0.5 size-3 rounded-full border-2 border-card bg-mint" /> : null}</span><span className="min-w-0"><span className="block truncate text-sm font-bold">{friend.other_user.display_name}</span><span className="block text-xs text-muted-foreground">{presenceLabel(friend.other_user)}</span></span></Link><Button variant="coral" size="sm" className="shrink-0" onClick={() => onChallenge(friend)} loading={busy === `challenge-${friend.other_user.id}`}><Swords className="size-3.5" aria-hidden /> Challenge</Button></div>)}</div> : <EmptyPanel icon={Users} title="No friends yet" detail="Search the Friends tab to build your circle." />}</CardContent>
    </Card>
    <Card>
    ...
    <details className="group rounded-lg border-2 border-foreground/15 bg-card">
      ...
      <div className="border-t-2 border-foreground/10 p-4"><p className="mb-3 text-sm text-muted-foreground">Accept trusted learners before inviting them to a duel.</p>{requests?.incoming.length ? <div className="grid gap-2">{requests.incoming.map((request) => <div key={request.id} className="flex items-center justify-between gap-3 rounded-md border-2 border-coral/30 bg-coral/5 p-3"><Link href={`/profile/${request.other_user.id}?from=combat`} className="flex min-w-0 items-center gap-2 rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-foreground"><Avatar name={request.other_user.display_name} avatarId={request.other_user.avatar_id} avatarUrl={request.other_user.avatar_url} size="sm" /><p className="truncate text-sm font-bold">{request.other_user.display_name}</p></Link><div className="flex shrink-0 gap-1"><Button variant="ghost" size="sm" onClick={() => onRespondRequest(request.id, 'declined')} disabled={busy === `request-${request.id}`}>No</Button><Button variant="accent" size="sm" onClick={() => onRespondRequest(request.id, 'accepted')} loading={busy === `request-${request.id}`}>Accept</Button></div></div>)}</div> : <EmptyPanel icon={Inbox} title="No requests waiting" detail="New friend requests will appear here." />}</div>
    </details>
  </div>
}
