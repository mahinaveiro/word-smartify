...
export function CombatMatchView({ matchId }: { matchId: string }) {
  ...
  const [quickPickerPosition, setQuickPickerPosition] = React.useState<number | null>(null)
  ...
  const quickPickerVisible = quickPickerOpen && currentMatch?.status === 'active' && quickPickerPosition === currentPosition
  ...
  const requestExit = () => {
    if (!currentMatch) return
    if (currentMatch.status === 'active' || currentMatch.status === 'waiting' || currentMatch.status === 'ready') setShowLeaveConfirm(true)
    else router.push('/combat')
  }

  const leaveLobby = () => {
    if (!currentMatch || (currentMatch.status !== 'waiting' && currentMatch.status !== 'ready')) return
    if (isHost) {
  ...
  const confirmExit = () => {
    ...
    if (currentMatch.status === 'active') {
      void run('leave', async () => {
        await postCombat({ action: 'leave', matchId })
        try {
          window.sessionStorage.setItem('combat-leave-reminder', matchId)
        } catch {
  ...
  const toggleQuickPicker = () => {
    if (quickPickerOpen) {
      ...
      setQuickPickerPosition(currentPosition)
      setQuickPickerOpen(true)
    }
  }

  const errorModal = <Modal open={Boolean(error)} onClose={() => setError(null)} title="Combat action could not be completed" className="max-w-sm">
  ...
  return <div className="mx-auto flex min-h-dvh w-full max-w-4xl flex-col gap-3 px-3 pb-5 pt-3 sm:gap-4 sm:px-5 sm:pt-5 md:gap-6 md:px-0 md:pb-10">
    ...
    {errorModal}
    {currentMatch.status === 'waiting' || currentMatch.status === 'ready' ? <LobbyPanel match={currentMatch} userId={user?.id} isHost={isHost} opponent={opponent} bothReady={bothReady} busy={busy} copied={copied} onCopy={() => void copyCode()} onReady={toggleReady} onStart={start} onCancel={leaveLobby} /> : null}
    {currentMatch.status === 'active' ? <ActiveMatchPanel match={currentMatch} question={freshQuestion} loading={question.isLoading || !freshQuestion} selected={selectedForQuestion} remainingMs={graceRemainingMs} opponentSubmitted={hasOpponentSubmission} onSelect={selectAnswer} onSubmit={submitAnswer} hasSubmitted={hasSubmittedForQuestion} onRefresh={refresh} onReport={() => setShowReport(true)} onQuickMessage={sendQuickMessage} quickMessagesBySender={latestQuickMessagesBySender} currentTimeMs={clockMs} quickPickerOpen={quickPickerVisible} onToggleQuickPicker={toggleQuickPicker} busy={busy} myPlayer={myPlayer} otherPlayer={opponent} /> : null}
    ...
    <Modal open={showReport} onClose={() => setShowReport(false)} title="Report this match" description="Use this for a broken question, connection issue, cheating concern, or harassment."><ReportForm matchId={matchId} onDone={() => setShowReport(false)} /></Modal>
      <Modal open={showLeaveConfirm} onClose={() => setShowLeaveConfirm(false)} title={currentMatch.status === 'active' ? 'Leave this match?' : isHost ? 'Cancel this room?' : 'Leave this room?'} description={currentMatch.status === 'active' ? 'Leaving ends your run. In a wagered match, your 100 XP stake may be lost.' : isHost ? 'This closes the private room for everyone and settles any protected stake safely.' : 'You will leave the waiting room. The host can still invite another player.'} className="max-w-sm">
      <div className="flex flex-col gap-3"><div className="rounded-md border-2 border-coral/40 bg-coral/10 p-3 text-sm leading-5"><p className="font-heading font-bold">{currentMatch.status === 'active' ? 'Your opponent is waiting.' : isHost ? 'This room will close.' : 'Leave the waiting room?'}</p><p className="mt-1 text-muted-foreground">{currentMatch.status === 'active' ? 'Only leave if you are sure. You can return to Combat history afterward.' : isHost ? 'The room will stop accepting players.' : 'You can return to Combat and join another room.'}</p></div><div className="grid grid-cols-2 gap-2"><Button variant="outline" size="sm" onClick={() => setShowLeaveConfirm(false)}>Stay</Button><Button variant="accent" size="sm" onClick={confirmExit} loading={busy === 'cancel' || busy === 'leave'}>{currentMatch.status === 'active' ? 'Leave match' : isHost ? 'Cancel room' : 'Leave room'}</Button></div></div>
    </Modal>
  </div>
}
...
function CombatResultPanel({ result, userId, onBack, onRematch, rematchBusy }: { result: CombatResult; userId?: string; onBack: () => void; onRematch: () => void; rematchBusy: boolean }) {
  ...
  return <div className="mx-auto flex w-full max-w-2xl flex-col gap-3 px-3 pb-8 pt-3 sm:gap-5 sm:px-0 sm:pb-20 sm:pt-4">
    <section className={cn('relative overflow-hidden rounded-lg border-2 border-foreground px-4 py-5 shadow-brutal sm:px-8 sm:py-6', won ? 'bg-mint/20' : draw ? 'bg-muted' : 'bg-coral/10')}><div className="pointer-events-none absolute -right-10 -top-12 size-32 rounded-full border-[14px] border-coral/30" /><div className="pointer-events-none absolute -bottom-16 -left-8 size-36 rounded-full border-[12px] border-mint/30" /><div className="relative text-center"><button type="button" onClick={onBack} className="press absolute left-0 top-0 z-20 inline-flex size-9 shrink-0 items-center justify-center rounded-md border-2 border-foreground bg-card text-foreground shadow-brutal-sm outline-none transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background" aria-label="Close result" title="Close result"><X className="size-5" strokeWidth={2.5} aria-hidden="true" /></button><p className={cn('inline-flex items-center rounded-full border-2 border-foreground px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] shadow-brutal-sm sm:text-xs', won ? 'bg-mint text-mint-foreground' : draw ? 'bg-card text-foreground' : 'bg-coral text-coral-foreground')}>{won ? 'Victory secured' : draw ? 'Dead even' : abandonedLoss || result.outcome === 'loss' ? 'Good fight' : 'Combat result'}</p><h1 className="mt-2 font-heading text-3xl font-black leading-none tracking-tight sm:text-4xl">{won ? 'You own this round.' : draw ? 'No one backed down.' : abandonedLoss || result.outcome === 'loss' ? 'A close one.' : 'The duel is over.'}</h1><p className="mx-auto mt-2 max-w-md text-sm font-semibold leading-5 text-foreground/80 sm:leading-6">{outcomeReason} {wagerReason}</p></div>{winner ? <div className="relative mx-auto mt-3 h-36 w-28 sm:mt-5 sm:h-56 sm:w-36"><Image src="/assets/crown.gif" alt="" width={128} height={88} unoptimized className="pointer-events-none absolute left-1/2 top-1.5 z-10 h-auto w-20 max-w-none -translate-x-1/2 object-contain drop-shadow-md sm:top-3 sm:w-32" /><div className="absolute bottom-0 left-1/2 size-[5.5rem] -translate-x-1/2 rounded-full border-4 border-foreground bg-coral p-1.5 shadow-brutal-md sm:size-32 sm:border-4 sm:p-2"><Avatar name={winner.profile.display_name} avatarId={winner.profile.avatar_id} avatarUrl={winner.profile.avatar_url} size="xl" className="size-full text-3xl sm:text-4xl" /></div></div> : <div className="relative mx-auto mt-3 grid size-16 place-items-center rounded-full border-2 border-foreground bg-card shadow-brutal-sm sm:mt-5 sm:size-20"><Users className="size-7 sm:size-9" aria-hidden /></div>}<div className="relative mt-3 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-end gap-2 sm:mt-5 sm:gap-5"><ResultPlayerLane player={myPlayer} score={result.my_score} accuracy={result.my_accuracy} isWinner={Boolean(winnerId && winnerId === myPlayer?.user_id)} label="You" align="right" /><div className="flex flex-col items-center gap-1 pb-3"><span className="grid size-10 place-items-center rounded-full border-2 border-foreground bg-foreground font-heading text-[11px] font-black text-primary-foreground shadow-brutal-sm sm:size-11 sm:text-xs">VS</span><span className="font-heading text-xl font-black tabular-nums sm:text-2xl">{result.my_score} : {result.opponent_score}</span></div><ResultPlayerLane player={opponent} score={result.opponent_score} accuracy={result.opponent_accuracy} isWinner={Boolean(winnerId && winnerId === opponent?.user_id)} label="Opponent" align="left" /></div><div className="relative mt-3 flex flex-wrap justify-center gap-2"><Button type="button" size="sm" variant="outline" className="min-h-10 bg-card px-4 text-sm font-bold shadow-brutal-sm sm:px-5" onClick={() => void handleShare()}><Share2 className="size-4" aria-hidden /> Share result</Button></div>{shareMessage ? <p className="relative mt-2 text-center text-xs font-semibold text-foreground/80" role="status" aria-live="polite">{shareMessage}</p> : null}</section>
...
function ResultPlayerLane({ player, score, accuracy, isWinner, label, align }: { player?: CombatMatch['players'][number]; score: number; accuracy: number; isWinner: boolean; label: string; align: 'left' | 'right' }) {
  ...
  return <div className={cn('flex min-w-0 flex-col gap-1.5', textAlign, isWinner && 'motion-safe:-translate-y-1')}><div className="relative"><Avatar name={player?.profile.display_name ?? label} avatarId={player?.profile.avatar_id} avatarUrl={player?.profile.avatar_url} size="md" className={cn(isWinner ? 'border-4 border-coral shadow-brutal-sm' : 'border-2 border-foreground/30', 'size-11 bg-card sm:size-12')} /></div><div className="min-w-0 max-w-full"><p className="truncate font-heading text-sm font-black sm:text-base">{player?.profile.display_name ?? label}</p><p className="text-[10px] font-bold uppercase tracking-wide text-foreground/70 sm:text-[10px]">{isWinner ? 'Winner' : label}</p></div><div className={cn('rounded-md border-2 px-3 py-1.5 shadow-brutal-sm', isWinner ? 'border-foreground bg-coral text-foreground' : 'border-foreground/30 bg-card text-foreground')}><span className="font-heading text-2xl font-black tabular-nums sm:text-3xl">{score}</span><span className="ml-1 text-[10px] font-bold uppercase sm:text-[10px]">{accuracy}%</span></div></div>
}
