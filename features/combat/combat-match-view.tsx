...
export function CombatMatchView({ matchId }: { matchId: string }) {
  ...
  const rematch = () => void run('rematch', async () => {
    if (!currentMatch) return
    const rematchMatch = await postCombat<CombatMatch>({ action: 'create', preset: currentMatch.preset, questionCount: currentMatch.question_count, timeLimitSeconds: currentMatch.time_limit_seconds })
