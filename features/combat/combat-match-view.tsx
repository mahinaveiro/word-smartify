...
function CombatResultPanel({ result, userId, onBack, onRematch, rematchBusy }: { result: CombatResult; userId?: string; onBack: () => void; onRematch: () => void; rematchBusy: boolean }) {
  ...
  return <div className="mx-auto flex w-full max-w-3xl flex-col gap-5 pb-20">
    <section className={cn('relative overflow-hidden rounded-lg border-2 border-foreground px-3 py-6 shadow-brutal sm:px-8 sm:py-8', won ? 'bg-mint/20' : draw ? 'bg-muted' : 'bg-coral/10')}>
      <div className="pointer-events-none absolute -right-10 -top-12 size-32 rounded-full border-[14px] border-coral/30" />
      <div className="pointer-events-none absolute -bottom-16 -left-8 size-36 rounded-full border-[12px] border-mint/30" />
      <div className="relative text-center"><p className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">{won ? 'Victory secured' : draw ? 'Dead even' : result.outcome === 'loss' ? 'The rematch is waiting' : 'Combat result'}</p><h1 className="mt-1 font-heading text-3xl font-black tracking-tight sm:text-4xl">{won ? 'You own this round.' : draw ? 'No one backed down.' : result.outcome === 'loss' ? 'A close one.' : 'The duel is over.'}</h1><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground">{outcomeReason} {wagerReason} Your learning progress stays safe.</p></div>
      {winner ? <div className="relative mx-auto mt-5 w-fit motion-safe:animate-[bounce_1.8s_ease-in-out_infinite]"><div className="absolute -inset-3 rounded-full border-2 border-coral/50" /><div className="relative rounded-full border-4 border-foreground bg-coral p-1.5 shadow-brutal-md"><Avatar name={winner.profile.display_name} avatarId={winner.profile.avatar_id} avatarUrl={winner.profile.avatar_url} size="xl" /></div><span className="absolute -right-2 -top-3 grid size-9 place-items-center rounded-full border-2 border-foreground bg-card text-coral shadow-brutal-sm"><Crown className="size-5" aria-hidden /></span></div> : <div className="relative mx-auto mt-5 grid size-20 place-items-center rounded-full border-2 border-foreground bg-card shadow-brutal-sm"><Users className="size-9" aria-hidden /></div>}
      <div className="relative mt-7 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-end gap-2 sm:gap-5">
        <ResultPlayerLane player={myPlayer} score={result.my_score} accuracy={result.my_accuracy} isWinner={Boolean(winnerId && winnerId === myPlayer?.user_id)} label="You" align="right" />
        <div className="flex flex-col items-center gap-1 pb-3"><span className="grid size-10 place-items-center rounded-full border-2 border-foreground bg-foreground font-heading text-xs font-black text-primary-foreground shadow-brutal-sm">VS</span><span className="font-heading text-lg font-black tabular-nums">{result.my_score} : {result.opponent_score}</span></div>
        <ResultPlayerLane player={opponent} score={result.opponent_score} accuracy={result.opponent_accuracy} isWinner={Boolean(winnerId && winnerId === opponent?.user_id)} label="Opponent" align="left" />
      </div>
      <div className="relative mt-5 flex flex-wrap justify-center gap-2"><Button type="button" size="sm" variant="outline" className="bg-card" onClick={() => void handleShare()}><Share2 className="size-4" aria-hidden /> Share result</Button></div>{shareMessage ? <p className="relative mt-2 text-center text-xs font-semibold text-muted-foreground" role="status" aria-live="polite">{shareMessage}</p> : null}
    </section>
    {wagered ? <div className="flex items-center justify-between gap-3 rounded-md border-2 border-foreground/15 bg-muted/30 px-4 py-3"><div className="flex min-w-0 items-center gap-2"><Coins className="size-4 shrink-0 text-coral" aria-hidden /><div className="min-w-0"><p className="text-sm font-bold">XP wager</p><p className="text-xs text-muted-foreground">{result.wager_status === 'refunded' ? 'Refunded to both players' : '100 XP each · 200 XP winner payout'}</p></div></div><span className={cn('shrink-0 font-heading font-black', result.my_xp_delta > 0 ? 'text-mint-foreground' : result.my_xp_delta < 0 ? 'text-coral-foreground' : 'text-muted-foreground')}>{result.my_xp_delta > 0 ? '+' : ''}{result.my_xp_delta} XP</span></div> : null}
    ...
    <div className="grid gap-2 sm:grid-cols-2"><Button variant="outline" onClick={onBack}><ArrowLeft className="size-4" aria-hidden /> Back to Combat</Button><Button variant="accent" onClick={onRematch} loading={rematchBusy}><SwordsIcon /> Rematch</Button></div>
  </div>
}
...
function ResultPlayerLane({ player, score, accuracy, isWinner, label, align }: { player?: CombatMatch['players'][number]; score: number; accuracy: number; isWinner: boolean; label: string; align: 'left' | 'right' }) {
  ...
  return <div className={cn('flex min-w-0 flex-col gap-2', textAlign, isWinner && 'motion-safe:-translate-y-1')}><div className="relative"><Avatar name={player?.profile.display_name ?? label} avatarId={player?.profile.avatar_id} avatarUrl={player?.profile.avatar_url} size="md" className={cn(isWinner ? 'border-4 border-coral shadow-brutal-sm' : 'border-2 border-foreground/20', 'bg-card')} />{isWinner ? <span className="absolute -bottom-1 -right-1 grid size-5 place-items-center rounded-full border-2 border-foreground bg-mint text-mint-foreground"><Crown className="size-3" aria-hidden /></span> : null}</div><div className="min-w-0 max-w-full"><p className="truncate font-heading text-sm font-black sm:text-base">{player?.profile.display_name ?? label}</p><p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{isWinner ? 'Winner' : label}</p></div><div className={cn('rounded-md border-2 px-2.5 py-1.5', isWinner ? 'border-foreground bg-coral text-coral-foreground' : 'border-foreground/15 bg-card')}><span className="font-heading text-2xl font-black tabular-nums sm:text-3xl">{score}</span><span className="ml-1 text-[10px] font-bold uppercase">{accuracy}%</span></div></div>
}
