...
function CombatResultPanel({ result, userId, onBack, onRematch, rematchBusy }: { result: CombatResult; userId?: string; onBack: () => void; onRematch: () => void; rematchBusy: boolean }) {
  ...
  return <div className="mx-auto flex w-full max-w-2xl flex-col gap-3 px-3 pb-8 pt-3 sm:gap-5 sm:px-0 sm:pb-20 sm:pt-4">
    <div className="flex items-center justify-between px-1 sm:px-0"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground sm:text-xs">Combat result</p><button type="button" onClick={onBack} className="press inline-flex size-9 shrink-0 items-center justify-center rounded-md border-2 border-foreground bg-card text-foreground shadow-brutal-sm outline-none transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background" aria-label="Close result" title="Close result"><X className="size-5" strokeWidth={2.5} aria-hidden="true" /></button></div>
    <section className={cn('relative overflow-hidden rounded-lg border-2 border-foreground px-4 py-5 shadow-brutal sm:px-8 sm:py-7', won ? 'bg-mint/20' : draw ? 'bg-muted' : 'bg-coral/10')}><div className="pointer-events-none absolute -right-10 -top-12 size-32 rounded-full border-[14px] border-coral/30" /><div className="pointer-events-none absolute -bottom-16 -left-8 size-36 rounded-full border-[12px] border-mint/30" /><div className="relative text-center"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground sm:text-xs">{won ? 'Victory secured' : draw ? 'Dead even' : result.outcome === 'loss' ? 'Good fight' : 'Combat result'}</p><h1 className="mt-1 font-heading text-2xl font-black leading-tight tracking-tight sm:text-4xl">{won ? 'You own this round.' : draw ? 'No one backed down.' : result.outcome === 'loss' ? 'A close one.' : 'The duel is over.'}</h1><p className="mx-auto mt-2 max-w-xl text-sm leading-5 text-muted-foreground sm:mt-2 sm:text-sm sm:leading-6">{outcomeReason} {wagerReason}</p></div>{winner ? <div className="relative mx-auto mt-4 h-36 w-28 sm:mt-6 sm:h-56 sm:w-36"><Image src="/assets/crown.gif" alt="" width={128} height={88} unoptimized className="pointer-events-none absolute left-1/2 top-3 z-10 h-auto w-20 max-w-none -translate-x-1/2 object-contain sm:top-0 sm:w-32" /><div className="absolute bottom-0 left-1/2 size-20 -translate-x-1/2 rounded-full border-4 border-foreground bg-coral p-1.5 shadow-brutal-md sm:size-32 sm:border-4 sm:p-2"><Avatar name={winner.profile.display_name} avatarId={winner.profile.avatar_id} avatarUrl={winner.profile.avatar_url} size="xl" className="size-full text-3xl sm:text-4xl" /></div></div> : <div className="relative mx-auto mt-4 grid size-16 place-items-center rounded-full border-2 border-foreground bg-card shadow-brutal-sm sm:mt-6 sm:size-20"><Users className="size-7 sm:size-9" aria-hidden /></div>}<div className="relative mt-4 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-end gap-2 sm:mt-6 sm:gap-5"><ResultPlayerLane player={myPlayer} score={result.my_score} accuracy={result.my_accuracy} isWinner={Boolean(winnerId && winnerId === myPlayer?.user_id)} label="You" align="right" /><div className="flex flex-col items-center gap-1 pb-3"><span className="grid size-9 place-items-center rounded-full border-2 border-foreground bg-foreground font-heading text-[10px] font-black text-primary-foreground shadow-brutal-sm sm:size-10 sm:text-xs">VS</span><span className="font-heading text-lg font-black tabular-nums">{result.my_score} : {result.opponent_score}</span></div><ResultPlayerLane player={opponent} score={result.opponent_score} accuracy={result.opponent_accuracy} isWinner={Boolean(winnerId && winnerId === opponent?.user_id)} label="Opponent" align="left" /></div><div className="relative mt-4 flex flex-wrap justify-center gap-2"><Button type="button" size="sm" variant="outline" className="bg-card" onClick={() => void handleShare()}><Share2 className="size-4" aria-hidden /> Share result</Button></div>{shareMessage ? <p className="relative mt-2 text-center text-xs font-semibold text-muted-foreground" role="status" aria-live="polite">{shareMessage}</p> : null}</section>
